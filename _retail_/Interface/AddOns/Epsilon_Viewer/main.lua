...
local addonFolder = "Interface\\AddOns\\" .. addonName
local assetsFolder = addonFolder .. "\\assets"
...
function g:InitializeOptions() --Interfaces options
	...
	if g.db.rows == nil then
		sliderRows:SetValue(2, true)
		g.db.rows = default.rows
	else
		sliderRows:SetValue(g.db.rows, true)
	end --Bad, i know
	...
	if g.db.columns == nil then
		sliderColumns:SetValue(4, true)
		g.db.columns = default.columns
	else
		sliderColumns:SetValue(g.db.columns, true)
	end --Bad, i know
...
local function getListFromEpsilon(filter, maxgobs) --C_Epsilon is the best
	...
	for i = 0, C_Epsilon.GODI_Search(filter) - 1 do -- 0 indexed
		if i > maxgobs then
			print("\124cFF4594C1[Epsilon_Viewer]\124r - Too much result (" .. maxgobs .. "+), ending the search.")
			break;
		end
...
local function ShowGobBrowser()
	...
	for i = 1, g.db.rows do
		...
		for j = 1, g.db.columns do
			...
			if columns > 4 then
				ButtonWSize = (ButtonWSize * (4 / columns))
				ButtonHSize = (ButtonHSize * (2 / rows))
			end
			...
			if #selectedGobs > 0 then
				...
				confirmButton:SetScript('OnClick', function()
					...
					if catalogGOBlist:GetValue() ~= 1 then
						...
						print("\124cFF4594C1[Epsilon_Viewer]\124r - You can't add or remove gobs from the Default list.")
					end
				end)
	...
	local searchButton = utils.SquareButton(gobFrame, 30, 30, assetsFolder .. "\\EpsiIndexSearch"); --SearchButton
