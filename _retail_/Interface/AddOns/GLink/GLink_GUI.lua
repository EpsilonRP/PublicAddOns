	...
	if IDType and ID and hyperlink and commandIndex then
		if IDType == "gameobject_GPS" then
			...
			if hyperlink == "[Teleport]" then
				output = string.format("Teleport to map %s%i|r (X: %s%#.3f|r, Y: %s%#.3f|r, Z: %s%#.3f|r)", GLink_Settings.colour, map, GLink_Settings.colour, x, GLink_Settings.colour, y,
					GLink_Settings.colour, z);
