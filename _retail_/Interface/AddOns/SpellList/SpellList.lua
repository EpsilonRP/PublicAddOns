...
--[[	Version Check	]]
--------------------------
local interfaceData = { GetBuildInfo() }
local interfaceVersion = tonumber(interfaceData[4])
local gamebuild = tonumber(interfaceData[2])
local postBackdropChange = false

local codebuild = tonumber(Title:match("%d+$"))
...
Addon.CacheControl = setmetatable({
	...
	__index = {
		...
		--	Filter Funcs
		FilterCheck = function(self, str, sid, sname)
			if not str then str = self.FilterString; end
			local filterArgs = { strsplit(",", str) }
			if not str:find("^(%s*%)$") then str = str:gsub("%s*%(%s*%)$", ""); end
			str = str:gsub("%s*%(%s*", "%(")
				:gsub("%s*%)%s*", "%)")
				:gsub("([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1")
				:lower();
			local spellName = ("%s"):format(sname):lower()
			local numMatchesNeeded = #filterArgs
			local numMatchesFound = 0
			for i = 1, numMatchesNeeded do
				local filterArg = filterArgs[i]
				if spellName:find(strtrim(filterArg)) then
					numMatchesFound = numMatchesFound + 1
				end
			end
			return (tonumber(str) == sid or (numMatchesFound >= numMatchesNeeded));
		end,
		...
		AddSpell = function(self, sid)
			...
			local name, rank, icon = GetSpellInfo(sid); -- this breaks icons if changed
			...
			if icon and #self.BlockedIcons > 0 then
				for i, j in ipairs(self.BlockedIcons) do
					if icon == j then
						icon = self.DefaultIcon; break;
					end
				end
			end

			local tbl = { SpellID = sid, Name = name, Icon = icon or self.DefaultIcon };
			self.SpellCache[sid] = tbl;
			self.MasterList[#self.MasterList + 1] = tbl;
			if self:FilterCheck(nil, sid, name) then
			...
			obj:HookScript("OnUpdate", function(self, elapsed)
				...
				while (elapsed - self.RunTime + debugprofilestop() / 1000 < limit or not self.FirstPass) and (self.FilterPos or self.ScanPos) do
					if self.FilterPos then
						...
						if self.FilterPos > #mlist then
							self.FilterPos = nil; break;
						end
					...
					elseif self.ScanPos then
						...
						if self.ScanPos > Addon.Options.MaxSpellID then
							self.ScanPos = nil; break;
						end
					end
					self.FirstPass = true;
				end
				...
				if self.FilterPos then
					...
					if ptr then txt = txt .. ("\nLast Spell: %u - |T%s:0|t %s"):format(ptr.SpellID, ptr.Icon, ptr.Name); end
				elseif self.ScanPos then
					self.LastAction = "Filter";
...
function SpellListFrame_OnLoad(self)
	...
	self.Keyhelp:SetText("Shift-Click: Cast | Alt-Click: Aura | CTRL-Click: Learn")
	...
	self.ShowDetails = function(self, sid)
		...
		for i = 1, 2 do
			...
			if dat[i] then
				...
				if postBackdropChange then -- SL Or newer, need to use adjusted backdrop system
					self.DetailTooltip[i].NineSlice:Hide()
				else
...
function SpellListFrame_OnEvent(self, event, ...)
	if event == "ADDON_LOADED" and (...) == Name then
		...
		SpellListButton:SetText("SpellList")
		SpellListButton:SetSize(100, 22)
		SpellListButton:SetPoint("RIGHT", SpellBookPrevPageButton, "LEFT", -320, 0)
		SpellListButton.tooltipText = "Open the SpellList AddOn to preview & search all the spells in the game."
		SpellListButton:SetScript("OnClick", function()
...
function SpellListFilterText_OnEnterPressed(self)
	...
	txt = txt:gsub("^%s+", "");
	--if final then txt = txt:gsub("%s+$", ""); end --Undefined global 'final' - this is either pulling from a leaky global and triggering unintended, or not triggering, both bad cases.
	txt = txt:gsub("%s%s+", " ");
	self:SetText(txt);
...
--------------------------
--[[	Epsilon Functions // MindScape	]]
--------------------------

function SpellList_castSpellFromList(spellID)
	SendChatMessage(".cast " .. spellID, GUILD)
end

function SpellList_auraSpellFromList(spellID)
	SendChatMessage(".aura " .. spellID, GUILD)
end

function SpellList_learnSpellFromList(spellID)
	SendChatMessage(".learn " .. spellID, GUILD)
end

function SpellList_unlearnSpellFromList(spellID)
	SendChatMessage(".unlearn " .. spellID, GUILD)
end
...
SlashCmdList["SPELLLIST"] = function(msg)
	...
	if arg[1] then
		...
		elseif cmd == "scanstop" then
			...
			print("     filter - Sets filter to given spell name (Leave 'Args' blank to reset) - Args accept comma delimited multiple search terms.");
