...
function EpsilonAuraManagerAuraEditor_SelectSearch(index, isSpellID)
    ...
    if isSpellID then
        ...
        if name then
            buff = {}
            buff = { name = name, icon = icon, spellDesc = GetSpellDescription(buff.spellID, true), spellID = spellID, desc =
            EpsilonAuraManagerAuraEditor_GetSpellTooltip(spellID) }
        end
    else
...
function EpsilonAuraManagerAuraEditor_UpdateSearchPreview(isSearchDone)
    ...
    EpsilonAuraManagerAuraEditor.Search.searchIndicator:SetPoint("TOP",
        EpsilonAuraManagerAuraEditor.Search.searchPreview[lastShownEntry], "BOTTOM")
    EpsilonAuraManagerAuraEditor.Search.showAllResults:SetPoint("TOP",
        EpsilonAuraManagerAuraEditor.Search.searchPreview[lastShownEntry], "BOTTOM")
...
function EpsilonAuraManagerAuraEditor_FixSearchPreviewBottomBorder()
    ...
    if lastShownButton ~= nil then
        EpsilonAuraManagerAuraEditor.Search.searchPreviewContainer.botRightCorner:SetPoint("BOTTOM", lastShownButton,
            "BOTTOM", 0, -8);
        EpsilonAuraManagerAuraEditor.Search.searchPreviewContainer.botLeftCorner:SetPoint("BOTTOM", lastShownButton,
            "BOTTOM", 0, -8);
    else
        EpsilonAuraManagerAuraEditor_HideSearchPreview();
    end
...
function EpsilonAuraManagerAuraEditor_OpenColorTagPicker(self)
    local parent = self:GetParent()
    local returnFunc = function(hex, r, g, b)
        local editBox = parent.buffDesc.EditBox

            local colorMixin = CreateColor(r, g, b)
            editBox:Insert("||c" .. colorMixin:GenerateHexColor() .. " " .. "||r")
            EpsilonLibColourPicker_Close()
    end
    EpsilonLibColourPicker_Open(returnFunc)
end
