...
local splashVersions = {
	["45745"] = {tex = "WhatsNew3", widthOffset = 0.12, heightOffset = 0, realWidth = 1024, realHeight = 512, regions = {
		{ x1 = 150, y1 = 189, x2 = 509, y2 = 333, title = "New Lands!", lines = "Experience the Shadowlands like never before!\n\rClick to choose a new zone to teleport to!", 
			callback = function(self)

				if not self.zones then self.zones = {
					...
					{ text = "Tazavesh", notCheckable = true, func = function() cmd("tele tazavesh_instance") end },
		...
		{ x1 = 512, y1 = 189, x2 = 871, y2 = 333, title = "New Character Options!", lines = "And did we mention, '.cheat barber' is working again!\n\rClick to start customizing!", callback = function() hideSplash(); cmd("cheat barber") end},
		{ x1 = 150, y1 = 338, x2 = 389, y2 = 482, title = "Object Viewer!", lines = "Search, View, and Catalogue objects to your hearts content, thanks to our new Object Viewer AddOn from Warli!\n\rClick to open the viewer!", callback = function() if IsAddOnLoaded("Gobviewer") then SlashCmdList["GOB"]() else print("Gobviewer doesn't seem to be loaded. Check you have the AddOn enabled!") end end},
		{ x1 = 392, y1 = 338, x2 = 632, y2 = 482, title = "Epsilon Editor Updated!", lines = "The Epsilon Editor has been overhauled. First things first, it now works on Windows 11 again!\n\rClick to open the editor!", callback = function() clickMMIcon("Epsilon Editor") end},
		{ x1 = 635, y1 = 338, x2 = 873, y2 = 482, title = "What is more? MORE!", lines = "Stuff!"},
	}},
}
...
frame.SetSplash = function(self, splash)
	...
	for _,regionData in ipairs(splashData.regions) do
		...
		f:SetPoint("TOPLEFT", (regionData.x1 - splashXAdjustment)*splashXMult, -(regionData.y1-splashYAdjustment)*splashYMult)
		f:SetPoint("BOTTOMRIGHT", self, "TOPLEFT", (regionData.x2 - splashXAdjustment)*splashXMult, -(regionData.y2-splashYAdjustment)*splashYMult)
		f:Show()
	end
