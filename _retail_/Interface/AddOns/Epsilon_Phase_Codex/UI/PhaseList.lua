...
local EpsilonPhasesPhaseListPublicTab = CreateFrame("Button", "EpsilonPhasesPhaseListPublicTab", EpsilonPhasesPhaseListFrame)
EpsilonPhasesPhaseListPublicTab:SetSize(150, 37)
...
EpsilonPhasesPhaseListMallTab:SetSize(150, 37)
...
local EpsilonPhasesPhaseListMallAdminButton = CreateFrame("Button", nil, EpsilonPhasesPhaseListFrame, "IconButtonTemplate")
        ...
        EpsilonLib.Utils.GenericDialogs.CustomConfirmation({
            ...
            callback = function()
                EpsilonLib.Utils.GenericDialogs.CustomInput({
                    ...
                    subText = "Optionally, you can add a teleport point name by adding a colon and the name after the phase ID\n(e.g. 123:TeleportName)",
                    acceptText = "Add",
                    cancelText = CANCEL,
...
local EpsilonPhasesPhaseListPrivateTab = CreateFrame("Button", EpsilonPhasesPhaseListPrivateTab, EpsilonPhasesPhaseListFrame)
EpsilonPhasesPhaseListPrivateTab:SetSize(150, 37)
...
function EpsilonPhases:SetupPhaseList()
    allPhases = PhaseClass:GetOverviewList()
    local count = 0
...
local function SetCurrentActivePhase(index)
    ...
    if next(allPhases) ~= nil and currentActivePhase ~= nil then
        ...
        if phaseListItem ~= nil then
            _G["EpsilonPhasesPhaseListPhaseItem" .. currentActivePhase].ActiveTexture:SetTexture(EpsilonPhases.ASSETS_PATH .. "/EpsiIndexObjectFrameHover")         
        end
...
local function SortPhaseList()
    ...
    phaseList = table.sort(phaseList, function(phase1, phase2)
        if EpsilonPhases.Favourites[phase1.data.id] and not EpsilonPhases.Favourites[phase2.data.id] then
            return true
        end
        return false
    end)
