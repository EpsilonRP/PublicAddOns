...
print("\124cFF4594C1[GobViewer]\124r | /gobviewer - /gobv")
...
local function OnEvent(self, event, addOnName)
   if addOnName == "Gobviewer" then
      GobViewerDB = GobViewerDB or {}
      self.db = GobViewerDB 
...
function g:InitializeOptions() --Interfaces options
   ...
   self.panel.name = "GobViewer"

   local OptionsLabel = StdUi:Label(self.panel, "GobViewer - Options")
   ...
   local sliderMaxGobs = CreateFrame("Slider", "Max GOB Result", self.panel, "OptionsSliderTemplate")
   sliderMaxGobs:SetPoint("TOP", 0, -160)
   ...
   if g.db.maxgobs == nil then sliderMaxGobs:SetValue(5000, true) g.db.maxgobs=default.maxgobs else sliderMaxGobs:SetValue(g.db.maxgobs, true) end --Bad, i know
   ...
   local sliderMaxGobsLabel = StdUi:Label(sliderMaxGobs, "Max Gob in Result: "..g.db.maxgobs)
...
local function getListFromEpsilon(filter, maxgobs) --C_Epsilon is the best
   ...
   for i = 0, C_Epsilon.GODI_Search(filter)-1 do -- 0 indexed
      if i > maxgobs then print("\124cFF4594C1[GobViewer]\124r - Too much result ("..maxgobs.."+), ending the search.") break; end
...
local function getGobName(fid)
   if fid == 1 then 
      return " " 
   elseif fid == 130736 then 
      return "talktomequestion_ltblue.m2" --Since i'm not using the GobList anymore, need to make sure of that.
   else
...
local function getGobList(filter, catalogValue, maxgobs)  --Filter to build resultTable
      ...
      for j = 1, maxGobInGrid do
         ...
         if #rawList < maxGobInGrid then
            ...
            for holder = 1, gobToAdd do
               tinsert(rawList, {fid = 130736}) --placeHolder (talktomequestion_ltblue.m2) if not enough gob, bad but it will do for now.
            end
         end
...
local function ShowGobBrowser() 
   ...
   local gobFrame = StdUi:Window(UIParent, 1000, 700, 'GameObject Viewer');
   gobFrame.titlePanel.label:SetTextColor(0.2705,0.5803,0.7568)
   gobFrame:SetMovable(true)
   gobFrame:EnableMouse(true)
   gobFrame:RegisterForDrag("LeftButton")
   gobFrame:SetPoint("CENTER");
   gobFrame:SetScript("OnDragStart", gobFrame.StartMoving)
   gobFrame:SetScript("OnDragStop", gobFrame.StopMovingOrSizing)
   gobFrame:Show()

   StdUi:EasyLayout(gobFrame, {columns = columns, padding = {top = 100, bottom = 60}}); --A lot of placeholder in there
   ...
   for i = 1, g.db.rows do 
      ...
      for j = 1, g.db.columns do
         ...
         g:SetModel(130736) --"interface/buttons/talktomequestion_ltblue.m2"
         g:SetSequence(122)
         ...
         gameObjectsGrid[i][j].buttonSelect:SetScript("OnClick", function()
            ...
            elseif gameObjectsGrid[i][j].buttonSelect.selected then
               ...
               print("\124cFF4594C1[GobViewer]\124r - Can't select the same gob twice.")
            end
         end)
         ...
         gameObjectsGrid[i][j].buttonSpawn:SetScript("OnClick", function(self, arg1) --Will tell us what button was used to click it.
            if arg1 == "LeftButton" then --Spawning
               ...
               local gobID
               if gameObjectsGrid[i][j]:GetModelFileID() == 130736 then gobID = 804602 else gobID = gameObjectsGrid[i][j].gobData.entry end --if talktomequestion_ltblue then entry is...
               print("\124cFF4594C1[GobViewer]\124r - Spawning : "..gobName)
               SendChatMessage(".gob spawn "..gobID, "GUILD")
            else --Lookup
               --gameObjectsGrid[i][j].entryDropdown:ToggleOptions()
               local gobName = getGobName(gameObjectsGrid[i][j]:GetModelFileID())
               print("\124cFF4594C1[GobViewer]\124r - Searching : "..gobName)
   ...
   catalogListScrollDown.OnValueChanged = function(self, value)
      ...
      if value > 1 then --if not default, show everything in the catalog
         ...
         currentGobList = divideGobList(getGobList(" ", value, maxgobs), columns, rows)
   ...
   local function deleteCatalog(catalogValue)
      ...
      if catalogValue > 1 and catalogValue <= #g.db.catalogNameList then
         ...
         print("\124cFF4594C1[GobViewer]\124r - Catalog '"..deletedName.."' deleted successfully.")

      else
         print("\124cFF4594C1[GobViewer]\124r - Can't delete the default catalog.")
      end
   end
   ...
   local function addCatalog(name)
      ...
      if name ~= nil and name:len() > 2 then 
         ...
         catalogListScrollDown:SetOptions(g.db.catalogNameList)
         print("\124cFF4594C1[GobViewer]\124r - Catalog '"..name.."' added successfully !")
         return value
      else 
         print("\124cFF4594C1[GobViewer]\124r - Catalog could not be added, please enter a valid name.")
         return nil
      end
   ...
   catalogNewOrDelete:SetScript("OnClick", function()
      ...
      local catalogManager = StdUi:Window(UIParent, 300, 500, "Catalog Manager")
      catalogManager.titlePanel.label:SetTextColor(0.2705,0.5803,0.7568)
      catalogManager:SetMovable(true)
      catalogManager:EnableMouse(true)
      catalogManager:RegisterForDrag("LeftButton")
      catalogManager:SetPoint("CENTER");
      catalogManager:SetScript("OnDragStart", catalogManager.StartMoving)
      catalogManager:SetScript("OnDragStop", catalogManager.StopMovingOrSizing)
      ...
      renameCatalogButton:SetScript('OnClick', function()
         ...
         if renameValue > 1 then
            ...
            print("\124cFF4594C1[GobViewer]\124r - Changed name of "..oldName.." to "..newName..". ")
            catalogManagerList:SetOptions(g.db.catalogNameList)
            if renameValue == catalogListScrollDown:GetValue() then catalogListScrollDown:SetValue(renameValue, newName) end
            catalogListScrollDown:SetOptions(g.db.catalogNameList)
            catalogManager.closeBtn:Click()
         else
            print("\124cFF4594C1[GobViewer]\124r - You can't rename the default catalog. ")
         end
   ...
   local addOrRemoveFromCatalog = StdUi:Button(gobFrame, 80, 30, 'Add/Rmv'); -- Add or Remove Gob(s) from a catalog
   addOrRemoveFromCatalog:SetScript("OnClick", function()
   ...
   if #selectedGobs > 0 then
      ...
      local catalogGOBWindow = StdUi:Window(UIParent, 320, (200+(status-2)*10), "Add or Remove")
      catalogGOBWindow.titlePanel.label:SetTextColor(0.2705,0.5803,0.7568)
      catalogGOBWindow:SetMovable(true)
      catalogGOBWindow:EnableMouse(true)
      catalogGOBWindow:RegisterForDrag("LeftButton")
      catalogGOBWindow:SetPoint("CENTER");
      catalogGOBWindow:SetScript("OnDragStart", catalogGOBWindow.StartMoving)
      catalogGOBWindow:SetScript("OnDragStop", catalogGOBWindow.StopMovingOrSizing)
      ...
      confirmButton:SetScript('OnClick', function()
         ...
         if catalogGOBlist:GetValue() ~= 1 then
            ...
            if optionDropDown:GetValue() == 1 then --Add
               ...
               local addedGobs = "\124cFF4594C1[GobViewer]\124r - Those gobs were successfully added to the catalog : "
               local notAddedGobs = "\124cFF4594C1[GobViewer]\124r - Those gobs already are in the catalog : "
               ...
               if catalogListScrollDown:GetValue() == (value + 1) then --refresh if same catalog
                  ...
                  currentGobList = divideGobList(getGobList(" ", catalogListScrollDown:GetValue(), maxgobs), columns, rows)
                  updateGobGrid(currentGobList, 1)
               end
               ...
               local removedGobs = "\124cFF4594C1[GobViewer]\124r - Those gobs were successfully removed from the catalog : "
               local gobNotInUserCatalog = "\124cFF4594C1[GobViewer]\124r - Those gobs were not in the catalog : "
         ...
         else print("\124cFF4594C1[GobViewer]\124r - You can't add or remove gobs from the Default list.") end

      end)


      StdUi:GlueTop(addOrRemoveLabel, catalogGOBWindow, 0, -40);
      StdUi:GlueBelow(catalogGOBlist, addOrRemoveLabel, 0, -10);
      StdUi:GlueBelow(optionDropDown, catalogGOBlist, 0, -10);
      StdUi:GlueBelow(confirmButton, optionDropDown, 0, -20);


   else
      print("\124cFF4594C1[GobViewer]\124r - No GOB has been selected.")
   end
   ...
   local importButton = StdUi:Button(gobFrame, 65, 30, 'Import'); --Import a catalog (not is JSON)
   importButton:SetScript("OnClick", function()

      local importWindow = StdUi:Window(UIParent, 250, 180, "Catalog Import")
      importWindow.titlePanel.label:SetTextColor(0.2705,0.5803,0.7568)
   ...
   local exportButton = StdUi:Button(gobFrame, 65, 30, 'Export'); --Export a catalog (not is JSON)
   exportButton:SetScript("OnClick", function()
      ...
      if catalogValue > 1 then 
         ...
         local exportWindow = StdUi:Window(UIParent, 250, 125, "Export "..catalogName)
         exportWindow.titlePanel.label:SetTextColor(0.2705,0.5803,0.7568)
         ...
         print("\124cFF4594C1[GobViewer]\124r - You can't export the default catalog ! Trust me, you don't want to do that.")
      end
   ...
   searchBox:SetScript('OnEnterPressed', function()
      ...
      selectedCatalog(catalogListScrollDown:GetValue())
      currentGobList = divideGobList(getGobList(input, catalogListScrollDown:GetValue(), maxgobs), columns, rows)
      updateGobGrid(currentGobList, 1)
   ...
   local searchButton = StdUi:Button(gobFrame, 50, 30, 'Search'); --SearchButton
   ...
   StdUi:GlueTop(searchBox, gobFrame, 20, -40, 'LEFT');
   StdUi:GlueRight(searchButton, searchBox, 5, 0);
   StdUi:GlueRight(catalogNewOrDelete, searchButton, 5, 0);
   ...
   StdUi:GlueBottom(previousButton, gobFrame, 20, 10, "LEFT");
   StdUi:GlueBottom(nextButton, gobFrame, -20, 10, "RIGHT");


   gobFrame:DoLayout();
   updateNamesLabel()
   toogleOV(gobFrame)
   -- Making the model reappear after closing
...
SLASH_GOB1 = "/gobviewer"
SLASH_GOB2 = "/gobv"
SlashCmdList["GOB"] = function(msg)
   if isOVOpen then
      toogleOV()
   else ShowGobBrowser()
   end
