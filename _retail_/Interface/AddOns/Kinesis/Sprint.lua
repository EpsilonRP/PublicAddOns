...
local UseMessageFiltering = not C_Epsilon.SetSpeed
...
local function chatSpeedFilter(self,event,m,...)
	if MessagesToFilter > 0 then
		if m:find("You set the speed of.*"..playerName) or m:find("You set the fly speed of.*"..playerName) or m:find("You set all speeds of.*"..playerName) or m:find("You set the swim speed of.*"..playerName) then
    ...
    if speeds.currentSpeed > 0 or KinesisOptions.profiles[KinesisCharOptions.activeProfile].sprint.allowSprintWhenNotMoving then
        ...
        for k,v in pairs(sprintSpeedTypes) do
            ...
            if sprintEnabled and speed > 0 then
                if UseMessageFiltering then MessagesToFilter = MessagesToFilter + 1 end
                cmd(("mod speed %s %s"):format(k, speed))
                --print("KEY DOWN", k, "to", speed)
            end
        end
