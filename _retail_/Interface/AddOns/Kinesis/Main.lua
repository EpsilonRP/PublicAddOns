    ...
    if force then
        ...
        if jumpBind1 == "SPACE" or jumpBind2 == "SPACE" then
            if shiftSpaceBind == "TOGGLEWORLDSTATESCORES" then
                ...
                C_Timer.After(0, function() SaveBindings(GetCurrentBindingSet()) end)
                print("Kinesis | Shift+Space was bound (default). We've deleted this binding to enable sprint-jumping. This will only occur once (per character), and you may re-set your binding if you wish, but it will block jumping while shift is held down.")
            elseif shiftSpaceBind ~= "JUMP" then
                print("Kinesis | Shift+Space is bound to "..shiftSpaceBind..", which is a custom bind you have set. This bind will block the ability to sprint-jump (holding shift to sprint + space to jump). We advise you to remove this binding if you wish to sprint-jump. You can use the 'Fix Sprint+Jump' button in the '/kinesis' menu to fix it automatically.")
            end
        else
            print("Kinesis | Jump is not set to the Space Bar. Please note that any bindings with Shift+Jump Key will interfere with your ability to sprint-jump. We advise you remove such bindings if they are present, or revert to using the Space Bar. You can use the 'Fix Sprint+Jump' button in the '/kinesis' menu to remove any conflicting bindings automatically.")
        end
    end
end
...
function Kinesis:OnInitialize()
    ...
    ns.Sprint.registerChatFilter()
    ns.Flight.registerChatFilter()
